...
=head1 SYNOPSIS
 ...
 Single Cell Analysis of Five'End (SCAFE) Suite: --> tool.bk.pool.pl <--
...
my ($curntTimeStamp) = &timeStamp();#->1046
my $ARGVStr = join "\n", (&currentTime(), $scriptAbsPath, @ARGV);#->269
...
my ($lib_list_path, $max_thread, $genome, $outputPrefix, $outDir, $overwrite) = &readParameters();#->883
#<\section>
#====================================================================================================================================================#
...
&logCalledCMDAndScript($ARGVStr, $result_script_dir, $scriptAbsPath);#->384
&printStartOrFinishMessage("startMessage");#->738
my ($bedtools_bin) = &checkBedtoolsVersion();#->202
#<\section>
#====================================================================================================================================================#
...
my ($chrom_size_path) = &checkChromSizePath($genome);#->226
my ($chrom_size_hsh_ref) = &generateChromSizeHsh($chrom_size_path);#->287
my ($chunk_size) = &generateChunkSize($chrom_size_hsh_ref, $num_chunk);#->359
my ($chunk_info_hsh_ref) = &generateChunkInfoHsh($chrom_size_hsh_ref, $chunk_size, $result_tmp_dir);#->313
my ($lib_info_hsh_ref) = &readLibInfo($lib_list_path);#->858
&splitBedChunk($chunk_info_hsh_ref, $lib_info_hsh_ref, $max_thread, $result_tmp_dir);#->951
#<\section>
#====================================================================================================================================================#
...
&processPerChunk($lib_info_hsh_ref, $bedtools_bin, $max_thread, $chunk_info_hsh_ref);#->772
&poolChunkResults($chunk_info_hsh_ref, $result_bed_dir, $result_log_dir, $paramTag, $result_tmp_dir, $rm_tmp_file);#->489
#<\section>
#====================================================================================================================================================#
...
&printOutputFileListAndReadme($ARGVStr, $paramTag, $outDir);#->623
&printStartOrFinishMessage("finishMessage");#->738
#<\section>
#====================================================================================================================================================#
...
#		generateChunkInfoHsh, generateChunkSize, pileupCtss
#		poolChunkResults, poolLongCTSS, processPerChunk
#		readLibInfo, splitBedChunk
#
#====================================================================================================================================================#
...
#	dependOnSub: reportAndLogStatus|929
#	appearInSub: >none
...
#	subroutineCategory: unassigned
#	dependOnSub: reportAndLogStatus|929
#	appearInSub: poolChunkResults|489, processPerChunk|772
...
#	dependOnSub: >none
#	appearInSub: printStartOrFinishMessage|738, reportAndLogStatus|929
#	primaryAppearInSection: >none
#	secondaryAppearInSection: 2_defineOutDirPath|120, 5_finishingTasks|162
#	input: none
#	output: $runTime
#	toCall: my ($runTime) = &currentTime();
#	calledInLine: 91, 754, 758, 763, 767, 945, 946
#....................................................................................................................................................#
...
#	subroutineCategory: unassigned
#	dependOnSub: reportAndLogStatus|929
#	appearInSub: processPerChunk|772
#	primaryAppearInSection: >none
#	secondaryAppearInSection: 4_convert|153
...
#....................................................................................................................................................#
#	subroutineCategory: unassigned
#	dependOnSub: checkChunkResults|246, reportAndLogStatus|929
#	appearInSub: >none
...
#....................................................................................................................................................#
	...
	foreach my $chunk_num (sort {$a <=> $b} keys %{$chunk_info_hsh_ref}) {
		...
		my ($valid) = &checkChunkResults($chunk_info_hsh_ref, $chunk_num, $file_check_ary_ref);#->246
...
#	dependOnSub: >none
#	appearInSub: >none
#	primaryAppearInSection: 5_finishingTasks|162
#	secondaryAppearInSection: >none
...
#....................................................................................................................................................#
#	subroutineCategory: general
#	dependOnSub: currentTime|269
#	appearInSub: >none
#	primaryAppearInSection: 2_defineOutDirPath|120, 5_finishingTasks|162
#	secondaryAppearInSection: >none
#	input: $StartOrFinishMessage
#	output: none
#	toCall: &printStartOrFinishMessage($StartOrFinishMessage);
#	calledInLine: 134, 166
#....................................................................................................................................................#
...
#....................................................................................................................................................#
#	subroutineCategory: unassigned
#	dependOnSub: checkChunkResults|246, pileupCtss|409, poolLongCTSS|589, reportAndLogStatus|929
#	appearInSub: >none
#	primaryAppearInSection: 4_convert|153
#	secondaryAppearInSection: >none
			...
			sub {
				...
				foreach my $chunk_num (keys %{$itemForThrHsh_ref}) {
					...
					if ($valid eq 'yes') {
						...
						&poolLongCTSS($chunk_unsort_long_ctss_bed_path, $chunk_long_ctss_bed_path, $chunk_info_hsh_ref, $chunk_num);#->589
						&pileupCtss($chunk_long_ctss_bed_path, $chunk_collapse_ctss_bed_path, $chunk_ung_collapse_ctss_bed_path, $chunk_num);#->409
					}
					$num_proc++;
...
#	dependOnSub: currentTime|269
#	appearInSub: checkBedtoolsVersion|202, checkChunkResults|246, generateChunkInfoHsh|313, pileupCtss|409, poolChunkResults|489, poolLongCTSS|589, processPerChunk|772, splitBedChunk|951
#	primaryAppearInSection: >none
...
#	calledInLine: 218, 262, 336, 460, 515, 549, 606, 614, 806, 813, 816, 826, 843, 847, 989, 1013
#....................................................................................................................................................#
	...
	print "[".&currentTime()."] ".$message.$trailingSpaces.$lineEnd;#->269
	print $tmplog_fh "[".&currentTime()."] ".$message.$lineEnd if $lineEnd ne "\r";#->269
