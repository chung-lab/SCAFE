...
=head1 SYNOPSIS
 ...
 Single Cell Analysis of Five'End (SCAFE) Suite: --> tool.sc.pool.pl <--
...
my ($curntTimeStamp) = &timeStamp();#->1155
my $ARGVStr = join "\n", (&currentTime(), $scriptAbsPath, @ARGV);#->270
...
my ($lib_list_path, $max_thread, $genome, $outputPrefix, $outDir, $overwrite) = &readParameters();#->992
#<\section>
#====================================================================================================================================================#
...
&logCalledCMDAndScript($ARGVStr, $result_script_dir, $scriptAbsPath);#->387
&printStartOrFinishMessage("startMessage");#->844
my ($bedtools_bin) = &checkBedtoolsVersion();#->203
#<\section>
#====================================================================================================================================================#
...
my ($chrom_size_path) = &checkChromSizePath($genome);#->227
my ($chrom_size_hsh_ref) = &generateChromSizeHsh($chrom_size_path);#->288
my ($chunk_size) = &generateChunkSize($chrom_size_hsh_ref, $num_chunk);#->362
my ($chunk_info_hsh_ref) = &generateChunkInfoHsh($chrom_size_hsh_ref, $chunk_size, $result_tmp_dir);#->314
my ($lib_info_hsh_ref) = &readLibInfo($lib_list_path);#->966
&splitBedChunk($chunk_info_hsh_ref, $lib_info_hsh_ref, $max_thread, $result_tmp_dir);#->1060
#<\section>
#====================================================================================================================================================#
...
&poolCellbarcode($lib_info_hsh_ref, $result_log_dir, $paramTag);#->527
&processPerChunk($lib_info_hsh_ref, $bedtools_bin, $max_thread, $chunk_info_hsh_ref);#->878
&poolChunkResults($chunk_info_hsh_ref, $result_bed_dir, $result_log_dir, $paramTag, $result_tmp_dir, $rm_tmp_file);#->573
#<\section>
#====================================================================================================================================================#
...
&printOutputFileListAndReadme($ARGVStr, $paramTag, $outDir);#->729
&printStartOrFinishMessage("finishMessage");#->844
#<\section>
#====================================================================================================================================================#
...
#
#	unassigned [n=12]:
#		checkChromSizePath, checkChunkResults, generateChromSizeHsh
#		generateChunkInfoHsh, generateChunkSize, pileupCBDedupUMI
#		poolCellbarcode, poolChunkResults, poolUMICBCTSS
...
#	dependOnSub: reportAndLogStatus|1038
#	appearInSub: >none
...
#	dependOnSub: >none
#	appearInSub: >none
#	primaryAppearInSection: 3_generate_chunk|140
#	secondaryAppearInSection: >none
#	input: $genome
...
#	subroutineCategory: unassigned
#	dependOnSub: reportAndLogStatus|1038
#	appearInSub: poolChunkResults|573, processPerChunk|878
#	primaryAppearInSection: >none
#	secondaryAppearInSection: 4_convert|153
...
#	dependOnSub: >none
#	appearInSub: printStartOrFinishMessage|844, reportAndLogStatus|1038
#	primaryAppearInSection: >none
#	secondaryAppearInSection: 2_defineOutDirPath|120, 5_finishingTasks|163
#	input: none
#	output: $runTime
#	toCall: my ($runTime) = &currentTime();
#	calledInLine: 91, 860, 864, 869, 873, 1054, 1055
#....................................................................................................................................................#
...
#	subroutineCategory: unassigned
#	dependOnSub: reportAndLogStatus|1038
#	appearInSub: processPerChunk|878
#	primaryAppearInSection: >none
#	secondaryAppearInSection: 4_convert|153
...
#	dependOnSub: reportAndLogStatus|1038
#	appearInSub: >none
#	primaryAppearInSection: 4_convert|153
#	secondaryAppearInSection: >none
#	input: $lib_info_hsh_ref, $paramTag, $result_log_dir
#	output: 
#	toCall: &poolCellbarcode($lib_info_hsh_ref, $result_log_dir, $paramTag);
#	calledInLine: 156
#....................................................................................................................................................#
	my ($lib_info_hsh_ref, $result_log_dir, $paramTag) = @_;

	my $pool_cellbarcode_path = "$result_log_dir/$paramTag.pool.cellbarcodes.tsv.gz";
	my $pool_suffix_path = "$result_log_dir/$paramTag.pool.suffix.txt";

	open SUFFIX, ">$pool_suffix_path";
	print SUFFIX join "", (join "\t", ('libID', 'suffix')), "\n";
	open OUTFILE, "| sort | gzip -c >$pool_cellbarcode_path";
	foreach my $libID (keys %{$lib_info_hsh_ref}) {
		my $suffix = $lib_info_hsh_ref->{$libID}{'suffix'};
		print SUFFIX join "", (join "\t", ($libID, $suffix)), "\n";
		my $cellbarcode_path = $lib_info_hsh_ref->{$libID}{'cellbarcode_path'};
		die "cellbarcode_path $cellbarcode_path is absent\n" if not -s $cellbarcode_path;
		if ($cellbarcode_path =~ m/\.gz$/) {
			open (FILEIN, " gzip -dc $cellbarcode_path|");
		} else {
			open (FILEIN, "<", $cellbarcode_path);
		}
		&reportAndLogStatus("pooling $libID cellbarcode", 10, "\n");#->1038

		while (<FILEIN>) {
			chomp;
			my ($cellbarcode) = $_;
			$cellbarcode =~ s/\-.+$//;
			$cellbarcode = $cellbarcode."-".$suffix;
			print OUTFILE join "", (join "\t", ($cellbarcode)), "\n";
		}
		close FILEIN;
	}
	close OUTFILE;
...
#....................................................................................................................................................#
#	subroutineCategory: unassigned
#	dependOnSub: checkChunkResults|247, reportAndLogStatus|1038
#	appearInSub: >none
#	primaryAppearInSection: 4_convert|153
#	secondaryAppearInSection: >none
...
#....................................................................................................................................................#
	...
	my $CB_ctss_bed_path = "$result_bed_dir/$paramTag.pool.CB.ctss.bed";
	...
	my $UMI_CB_ctss_bed_path = "$result_bed_dir/$paramTag.pool.UMI_CB.ctss.bed";
	...
	foreach my $chunk_num (sort {$a <=> $b} keys %{$chunk_info_hsh_ref}) {
		...
		my ($valid) = &checkChunkResults($chunk_info_hsh_ref, $chunk_num, $file_check_ary_ref);#->247
	...
	my $threadID = 1;
	my %itemForThrHsh = ();
	foreach my $itemID ($CB_ctss_bed_path, $ung_collapse_ctss_bed_path, $collapse_ctss_bed_path, $UMI_CB_ctss_bed_path) {
...
#	primaryAppearInSection: >none
#	secondaryAppearInSection: 4_convert|153
#	input: $chunk_UMI_CB_ctss_bed_path, $chunk_info_hsh_ref, $chunk_intersect_region_bed_path, $chunk_num, $chunk_unsort_UMI_CB_ctss_bed_path, $lib_info_hsh_ref
#	output: $chunk_UMI_CB_ctss_bed_path
#	toCall: my ($chunk_UMI_CB_ctss_bed_path) = &poolUMICBCTSS($lib_info_hsh_ref, $chunk_unsort_UMI_CB_ctss_bed_path, $chunk_UMI_CB_ctss_bed_path, $chunk_intersect_region_bed_path, $chunk_info_hsh_ref, $chunk_num);
#	calledInLine: 930
#....................................................................................................................................................#
	my ($lib_info_hsh_ref, $chunk_unsort_UMI_CB_ctss_bed_path, $chunk_UMI_CB_ctss_bed_path, $chunk_intersect_region_bed_path, $chunk_info_hsh_ref, $chunk_num) = @_;

	open OUTFILE, ">$chunk_unsort_UMI_CB_ctss_bed_path";
	foreach my $libID (keys %{$chunk_info_hsh_ref->{$chunk_num}{'lib_chunk_bed_path'}}) {
		...
		&reportAndLogStatus("pooling $libID in chunk $chunk_num UMI_CB ctss", 10, "\n");#->1038

		while (<FILEIN>) {
			chomp;
			my ($chrom, $end5Start, $end5End, $readname, $unencoded_G, $strand) = split /\t/;
			my ($CB, $UMI) = split /_/, $readname;
			$CB =~ s/\-.+$//;
			$CB = $CB."-".$suffix;
			$readname = join "_", ($CB, $UMI);
			print OUTFILE join "", (join "\t", ($chrom, $end5Start, $end5End, $readname, $unencoded_G, $strand)), "\n";
		}
		close FILEIN;
	...
	system "sort -k2,2n -k6,6 $chunk_unsort_UMI_CB_ctss_bed_path >$chunk_UMI_CB_ctss_bed_path";
	system "rm $chunk_unsort_UMI_CB_ctss_bed_path";
	
	close OUTFILE;
	
	return ($chunk_UMI_CB_ctss_bed_path);
}
...
#	dependOnSub: >none
#	appearInSub: >none
#	primaryAppearInSection: 5_finishingTasks|163
#	secondaryAppearInSection: >none
...
#....................................................................................................................................................#
#	subroutineCategory: general
#	dependOnSub: currentTime|270
#	appearInSub: >none
#	primaryAppearInSection: 2_defineOutDirPath|120, 5_finishingTasks|163
#	secondaryAppearInSection: >none
#	input: $StartOrFinishMessage
#	output: none
#	toCall: &printStartOrFinishMessage($StartOrFinishMessage);
#	calledInLine: 134, 167
#....................................................................................................................................................#
...
#....................................................................................................................................................#
#	subroutineCategory: unassigned
#	dependOnSub: checkChunkResults|247, pileupCBDedupUMI|412, poolUMICBCTSS|687, reportAndLogStatus|1038
#	appearInSub: >none
#	primaryAppearInSection: 4_convert|153
#	secondaryAppearInSection: >none
			...
			sub {
				...
				foreach my $chunk_num (keys %{$itemForThrHsh_ref}) {
					...
					if ($valid eq 'yes') {
						...
						my $chunk_collapse_ctss_bed_path = $chunk_info_hsh_ref->{$chunk_num}{'chunk_collapse_ctss_bed_path'};
						my $chunk_ung_collapse_ctss_bed_path = $chunk_info_hsh_ref->{$chunk_num}{'chunk_ung_collapse_ctss_bed_path'};
						my $chunk_intersect_region_bed_path = $chunk_info_hsh_ref->{$chunk_num}{'chunk_intersect_region_bed_path'};

						&poolUMICBCTSS($lib_info_hsh_ref, $chunk_unsort_UMI_CB_ctss_bed_path, $chunk_UMI_CB_ctss_bed_path, $chunk_intersect_region_bed_path, $chunk_info_hsh_ref, $chunk_num);#->687
						&pileupCBDedupUMI($chunk_UMI_CB_ctss_bed_path, $chunk_CB_ctss_bed_path, $chunk_collapse_ctss_bed_path, $chunk_ung_collapse_ctss_bed_path, $chunk_num);#->412
					}
					$num_proc++;
...
#....................................................................................................................................................#
	...
	while (<LIBINFO>) {
		chomp;
		my ($libID, $suffix, $UMI_CB_ctss_bed_path, $cellbarcode_path) = split /\t/;
		$lib_info_hsh_ref->{$libID}{'suffix'} = $suffix;
		$lib_info_hsh_ref->{$libID}{'UMI_CB_ctss_bed_path'} = $UMI_CB_ctss_bed_path;
		$lib_info_hsh_ref->{$libID}{'cellbarcode_path'} = $cellbarcode_path;
	}
...
#	dependOnSub: currentTime|270
#	appearInSub: checkBedtoolsVersion|203, checkChunkResults|247, generateChunkInfoHsh|314, pileupCBDedupUMI|412, poolCellbarcode|527, poolChunkResults|573, poolUMICBCTSS|687, processPerChunk|878, splitBedChunk|1060
#	primaryAppearInSection: >none
#	secondaryAppearInSection: 2_defineOutDirPath|120, 3_generate_chunk|140, 4_convert|153
#	input: $lineEnd, $message, $numTrailingSpace
#	output: 
#	toCall: &reportAndLogStatus($message, $numTrailingSpace, $lineEnd);
#	calledInLine: 219, 263, 337, 425, 481, 556, 605, 647, 705, 720, 912, 919, 922, 934, 951, 955, 1098, 1122
#....................................................................................................................................................#
	...
	print "[".&currentTime()."] ".$message.$trailingSpaces.$lineEnd;#->270
	print $tmplog_fh "[".&currentTime()."] ".$message.$lineEnd if $lineEnd ne "\r";#->270
		...
		($threadHsh{$threadID}) = threads->new(#---refer to http://www.perlmonks.org/?node_id=966781, the 
			...
			sub {
				...
				foreach my $libID (@{$itemForThrAry_ref}) {
					...
					my $UMI_CB_ctss_bed_path = $lib_info_hsh_ref->{$libID}{'UMI_CB_ctss_bed_path'};
					my $bed_FH_hsh_ref = {};
					
					if ($UMI_CB_ctss_bed_path =~ m/\.gz$/) {
						open (INFILE, " gzip -dc $UMI_CB_ctss_bed_path|");
					} else {
						open (INFILE, "<", $UMI_CB_ctss_bed_path);
					}
					while (<INFILE>) {
