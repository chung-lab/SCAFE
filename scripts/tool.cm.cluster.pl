...
=head1 SYNOPSIS
 ...
 Single Cell Analysis of Five'End (SCAFE) Suite: tool.cm.cluster.pl
...
my ($curntTimeStamp) = &timeStamp();#->1042
my $ARGVStr = join "\n", (&currentTime(), $scriptAbsPath, @ARGV);#->257
...
my ($cluster_ctss_bed_path, $count_ctss_bed_path_list, $count_scope_bed_path, $min_cluster_cpm, $min_cluster_count, $min_pos_count, $min_summit_count, $merge_dist, $min_num_sample_expr_cluster, $min_num_sample_expr_summit, $outputPrefix, $outDir, $overwrite) = &readParameters();#->853
#<\section>
#====================================================================================================================================================#
...
&printStartOrFinishMessage("startMessage");#->819
&logCalledCMDAndScript($ARGVStr, $resultScriptDir, $scriptAbsPath);#->426
#<\section>
#====================================================================================================================================================#
...
my ($paraclu_bin, $cut_sh_path) = &checkParacluPath();#->212
my ($bedtools_bin) = &checkBedtoolsVersion();#->181
my ($count_ctss_bed_hsh_ref) = &getCountCtssPath($count_ctss_bed_path_list, $count_scope_bed_path);#->275
my ($paraclu_input_path, $min_cluster_count_by_cpm) = &parseParacluInput($cluster_ctss_bed_path, $min_pos_count, $min_cluster_cpm, $resultParacluDir);#->538
my ($paraclu_bed_path) = &runParaclu($paraclu_input_path, $paraclu_bin, $cut_sh_path, $resultParacluDir, $resultPreMergeBedDir, $min_cluster_count, $min_cluster_count_by_cpm, $paramTag, $force_rerun);#->936
my ($paraclu_cluster_info_hsh_ref, $summit_bed_path) = &getTssClusterSummit($resultPreMergeBedDir, $paraclu_bed_path, $cluster_ctss_bed_path, $bedtools_bin, $paramTag);#->379
my ($tmp_merge_bed_file_path, $tssCluster_info_hsh_ref) = &mergeParacluClusters($bedtools_bin, $summit_bed_path, $merge_dist, $cluster_ctss_bed_path, $resultBedDir, $resultLogDir, $paramTag, $paraclu_cluster_info_hsh_ref, $min_summit_count);#->451
&getNumSampleExpressed($tmp_merge_bed_file_path, $tssCluster_info_hsh_ref, $count_ctss_bed_hsh_ref, $bedtools_bin);#->320
&printFinalBedAndLog($tmp_merge_bed_file_path, $tssCluster_info_hsh_ref, $resultBedDir, $resultLogDir, $paramTag, $min_num_sample_expr_cluster, $min_num_sample_expr_summit);#->593
#<\section>
#====================================================================================================================================================#
...
&printOutputFileListAndReadme($ARGVStr, $paramTag, $outDir);#->704
&printStartOrFinishMessage("finishMessage");#->819
#<\section>
#====================================================================================================================================================#
...
#
#	unassigned [n=8]:
#		checkParacluPath, getCountCtssPath, getNumSampleExpressed
...
#	dependOnSub: reportAndLogStatus|914
#	appearInSub: >none
#	primaryAppearInSection: 2_ReadInfoAndRun|126
...
#	secondaryAppearInSection: >none
#	input: none
#	output: $cut_sh_path, $paraclu_bin
#	toCall: my ($paraclu_bin, $cut_sh_path) = &checkParacluPath();
#	calledInLine: 129
#....................................................................................................................................................#
	
	my $dirPath = dirname(rel2abs($0));

	my $paraclu_bin = "paraclu";
	
	my $stdOut = `$paraclu_bin 2>&1`;
	if ($stdOut =~ m/paraclu: I need a minValue and a fileName/) {
		&reportAndLogStatus("Checking: paraclu found.", 0, "\n");#->914
	} else {
		$paraclu_bin = "$dirPath/../resources/bin/paraclu/paraclu";
		if ($stdOut =~ m/paraclu: I need a minValue and a fileName/) {
			&reportAndLogStatus("Checking: paraclu found.", 0, "\n");#->914
		} else {
	...
	my $radnom_string = "55C7128A";
	my $cut_sh_path = "paraclu-cut.sh";
	$stdOut = `$cut_sh_path $radnom_string 2>&1`;
	if ($stdOut =~ m/awk: fatal: cannot open/) {
		&reportAndLogStatus("Checking: paraclu-cut found.", 0, "\n");#->914
	} else {
		$cut_sh_path = "$dirPath/../resources/bin/paraclu/paraclu-cut.sh";
		if ($stdOut =~ m/awk: fatal: cannot open/) {
			&reportAndLogStatus("Checking: paraclu-cut found.", 0, "\n");#->914
		} else {
	...
	return ($paraclu_bin, $cut_sh_path);
}
sub currentTime {
#....................................................................................................................................................#
#	subroutineCategory: general
#	dependOnSub: >none
#	appearInSub: printStartOrFinishMessage|819, reportAndLogStatus|914
#	primaryAppearInSection: >none
#	secondaryAppearInSection: 1_defineHardCodedParam|103, 3_finishingTasks|142
#	input: none
#	output: $runTime
#	toCall: my ($runTime) = &currentTime();
#	calledInLine: 84, 835, 839, 844, 848, 930, 931
#....................................................................................................................................................#
...
#	dependOnSub: >none
#	appearInSub: >none
#	primaryAppearInSection: 1_defineHardCodedParam|103
#	secondaryAppearInSection: >none
...
#....................................................................................................................................................#
	...
	print OUTLOG join "", (join "\t", ('tssClusterID', 'chrom', 'num_paraclu_cluster', 'size', 'num_read', 'summit_pos', 'summit_read', 'num_sample_expr_cluster', 'stability')), "\n";
...
#	dependOnSub: >none
#	appearInSub: >none
#	primaryAppearInSection: 3_finishingTasks|142
#	secondaryAppearInSection: >none
...
#....................................................................................................................................................#
#	subroutineCategory: general
#	dependOnSub: currentTime|257
#	appearInSub: >none
#	primaryAppearInSection: 1_defineHardCodedParam|103, 3_finishingTasks|142
#	secondaryAppearInSection: >none
#	input: $StartOrFinishMessage
#	output: none
#	toCall: &printStartOrFinishMessage($StartOrFinishMessage);
#	calledInLine: 120, 146
#....................................................................................................................................................#
...
#	dependOnSub: >none
#	appearInSub: >none
#	primaryAppearInSection: 0_startingTasks|95
#	secondaryAppearInSection: >none
#	input: none
...
#	dependOnSub: currentTime|257
#	appearInSub: checkBedtoolsVersion|181, checkParacluPath|212, getCountCtssPath|275, getNumSampleExpressed|320, getTssClusterSummit|379, mergeParacluClusters|451, parseParacluInput|538, printFinalBedAndLog|593, runParaclu|936
#	primaryAppearInSection: >none
#	secondaryAppearInSection: 2_ReadInfoAndRun|126
#	input: $lineEnd, $message, $numTrailingSpace
#	output: 
#	toCall: &reportAndLogStatus($message, $numTrailingSpace, $lineEnd);
#	calledInLine: 199, 203, 230, 234, 244, 248, 288, 295, 304, 306, 336, 340, 369, 394, 413, 464, 465, 466, 468, 557, 558, 587, 588, 606, 692, 697, 955, 956, 957, 974, 978, 1007
#....................................................................................................................................................#
	...
	print "[".&currentTime()."] ".$message.$trailingSpaces.$lineEnd;#->257
	print $tmplog_fh "[".&currentTime()."] ".$message.$lineEnd if $lineEnd ne "\r";#->257
